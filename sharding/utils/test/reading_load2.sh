...
# The load on the individual shards
...
(( ($q < $h72) && (dt=$(shuf -i 3600000-$b1 -n 1))  && (delta=0)   )) || 
(( ($q < $w2)  && (dt=$(shuf -i $[$b1+1]-$b2 -n 1)) && (delta=$b1) )) || 
(( ($q < $m1)  && (dt=$(shuf -i $[$b2+1]-$b3 -n 1)) && (delta=$b2) )) || 
(( ($q < $m4)  && (dt=$(shuf -i $[$b3+1]-$b4 -n 1)) && (delta=$b3) ))

echo $q
date -u -d @$[$[$now-$delta-$dt]/1000] +%Y-%m-%d:%H:%M:%S
date -u -d @$[$[$now-$delta]/1000] +%Y-%m-%d:%H:%M:%S

request='db.legs.find({created:{$gte: new Date('$[$now-$delta-$dt]'),$lte: new Date('$[$now-$delta]')}},{_id:0,created:1}).hint({created:1}).sort({created:1}).limit(10)'
